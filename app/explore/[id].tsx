...
import { parseCIFData } from '@/services/ligandAPI';
...
export default function ExploreDetail() {
	...
	// Fetch SVG
	const { data: svgResp, loading: svgLoading, error: svgError, refetch: refetchSvg } = useFetch<string>(
		svgUrl,
		{ responseType: 'text' }
	);

	// Fetch CIF
	const { data: cifResp, loading: cifLoading, error: cifError, refetch: refetchCif } = useFetch<string>(
		cifUrl,
		{ responseType: 'text' }
	);
	...
	if (!id) {
		return (
			<View style={[globalStyles.container, { justifyContent: 'center', alignItems: 'center' }]}>
				<Text style={{ color: theme.colors.text.whiteLight }}>No ligand ID provided.</Text>
			</View>
		);
	}
				...
				<View style={styles.header}>
					<View style={styles.leftGroup}>
						<TouchableOpacity onPress={() => router.back()} style={styles.backButton}>
							<MCIcons name="arrow-left" size={22} color={theme.colors.text.white} />
						</TouchableOpacity>
		...
		return (
			<View style={globalStyles.container}>
				...
				<View style={styles.errorContainer}>
					...
					<Text style={styles.errorText}>{cifError || 'Unknown error'}</Text>
					<TouchableOpacity
	...
	return (
		...
		>
			...
			<View style={{ padding: theme.spacing.xl, paddingVertical: theme.spacing.sm }}>
			...
			<View style={styles.tabBar}>
				...
				>
					<MCIcons
						name="cube-outline"
						size={18}
						color={activeTab === '3D' ? '#4A90E2' : theme.colors.text.whiteLight}
					/>
					<Text style={[
						styles.tabText,
						activeTab === '3D' && styles.tabTextActive
					]}>
						3D View
					</Text>
				</TouchableOpacity>
				...
				>
					<MCIcons
						name="molecule"
						size={18}
						color={activeTab === '2D' ? '#4A90E2' : theme.colors.text.whiteLight}
					/>
					<Text style={[
						styles.tabText,
						activeTab === '2D' && styles.tabTextActive
					]}>
						2D Details
					</Text>
				</TouchableOpacity>
